...
export function PatientClient({ initialData }: Props) {
  ...
  // ✅ Función para actualizar header SOLO con localStorage
  const refreshHeader = async () => {
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        const role = sessionData?.user?.role || 'PACIENTE';
        const name = sessionData?.user?.name || '';
        const lastName = sessionData?.user?.lastname || '';
        const fullName = lastName ? `${name} ${lastName}` : name;
        
        localStorage.setItem('displayRole', role);
  ...
  const handleToggleStatus = async (id: string, currentStatus: string) => {
    ...
    if (result.success) {
      ...
      await refreshHeader();
      toast.success(`Paciente ${result.newStatus === 'Activo' ? 'activado' : 'desactivado'} correctamente`);
    } else {
